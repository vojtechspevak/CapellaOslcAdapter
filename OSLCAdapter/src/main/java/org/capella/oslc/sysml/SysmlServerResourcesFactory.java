...
public class SysmlServerResourcesFactory {
    ...
    //methods for SysmlPackage resource
    
    public static SysmlPackage createSysmlPackage(final String projectId, final String id)
    {
        return new SysmlPackage(constructURIForSysmlPackage(projectId, id));
    }
    
    public static URI constructURIForSysmlPackage(final String projectId, final String id)
    {
        String basePath = OSLC4JUtils.getServletURI();
        Map<String, Object> pathParameters = new HashMap<String, Object>();
        pathParameters.put("projectId", projectId);
        pathParameters.put("id", id);
        String instanceURI = "projects/{projectId}/packages/{id}";
    
        final UriBuilder builder = UriBuilder.fromUri(basePath);
        return builder.path(instanceURI).buildFromMap(pathParameters);
    }
    
    public static Link constructLinkForSysmlPackage(final String projectId, final String id , final String label)
    {
        return new Link(constructURIForSysmlPackage(projectId, id), label);
    }
    
    public static Link constructLinkForSysmlPackage(final String projectId, final String id)
    {
        return new Link(constructURIForSysmlPackage(projectId, id));
    }
